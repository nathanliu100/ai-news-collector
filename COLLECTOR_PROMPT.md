...
#### 2f-bis. 故事线追踪（连续剧新闻，⚠️ 必做）

**判断条件**：当前新闻是**过去 7 天内** 🟡/🔴 新闻的重大进展、续集、反转或结局。

**典型案例**：
- 企业法律诉讼（Musk vs Altman、Google vs DOJ 等）
- 并购/监管连续剧（Meta-Manus、OpenAI 重组等）
- 模型发布连锁反应（GPT-5 → GPT-5.5 → GPT-5.5-Turbo）
- 事故/事件追责（9 秒删库、大模型宕机事故等）

**输出格式**：在该新闻的摘要末尾（crew 评论之前）追加一个 blockquote 故事线小节：
...
**规则**：
- 故事线**最多 4 个节点**，超过则压缩为「X/XX 早期预告 → ... → **当前**」
- 每个节点 `YYYY-MM-DD 一句话关键事实`（5-15 字），用 `→` 连接
- **当前条目必须加粗**（`**4/28 陪审团确认**`），强调"这个故事到了什么阶段"
- 日期格式统一用 M/DD（省年份），节省显示空间
- 如果过去 7 天内找不到相关前置报道，**不要硬造**，省略这个小节
...
#### 2g-bis. Crew 质量控制（⚠️ 必做 — 避免评论区变成"背景噪音"）

**问题**：Crew 评论运行 10+ 天后，每个角色形成了可预测的回应模板，读者一扫就能猜出下一句要说什么。**当读者开始预测，评论就从"洞察"滑向"噪音"。**

**A. 禁用表达清单（7 天内不得重复使用）**

每位数字员工都有一批**已被过度使用**的口头禅/比喻/句式，在**最近 7 天内**已经说过的，当天禁止再用。如果找不到新表达，宁愿换一个评论角度，也不要套模板。

| Crew | 近期过载的表达（各选 1 条用过则当日禁用） |
|------|------------------------------------------|
| **Elon** | "第一性原理" / "曼哈顿计划时刻" / "X 年内消失 X%" / 笼统类比 Tesla/SpaceX/FSD 团队 |
| **Jobs** | "少即是多" / "工具民主化" / "真正的突破是让技术变得隐形" / 泛泛的"品味" |
| **万维钢** | "瑞士奶酪模型" / "相变" / "反直觉" / 笼统套用"统计学信号"但不给数据 |
| **Tim** | "创作者杠杆" / "我们做影视飓风的时候也用 AI" / "工具越强，品味越重要" / 笼统"AI 质感" |
| **MrBeast** | "可以做一期视频" / "XX 亿次播放" / "YouTube 的核心是人" / "100 个真人 vs 100 辆兰博基尼" |

**B. 质量弹性：允许 3 条 > 5 条水**

- 如果某条新闻对某个 Crew **不构成有效触发点**（比如 Tim 对纯金融并购、Elon 对设计工具细节），允许跳过该 Crew
- **3 条有增量的评论 > 5 条有 2 条水的评论**
- 最低不得少于 3 条（否则评论区没价值）；最高保持 5 条

**C. 信息增量三问（写每条评论前自检）**

1. **这条观点是我这个角色独有的吗？**（不是换个角度也能说）
2. **它引入了新信息或新角度吗？**（不是换个比喻重复前面已有的观点）
3. **它能在读者脑子里留下一个具体画面或具体数据吗？**（不是空泛的断言）

三问至少过 2 问才写；过不了就跳过这条。

**D. 每周一次"角色反转"（可选增强）**

每 5-7 天允许一次跨角色反串：比如让 Jobs 用 Elon 视角打一次战略牌，或让万维钢用 MrBeast 的流量思维分析一次。反串时加标注：

```markdown
<!-- crew:jobs -->（反串·战略视角）Jobs 在思考战略时会说... <strong>加粗核心观点</strong>。<!-- /crew -->
```

每天最多 1 次反串。不要连续两天反串同一个角色。
...
- 💎 **新闻标题** — 1 句话说清"为什么跟我们相关 / 能落地在哪（哪个项目 / 哪类素材）"
- 💎 **新闻标题** — 同上

⚠️ 规则：
- 即使当天没有 🔴，这里也不能省，挑 🟡 或 🟢 里最相关的即可
- 如果实在一条相关都没有，写 `- 今日无直接相关 AI 设计/素材动态，建议关注 [分类名] 的 [新闻标题]` 作为兜底
- 避免空话（不要写"这说明 AI 很重要"这种），每条要有"能用在哪"的落地提示
